...
% Spindle density requires input from sleep stage NREM2 epochs therefore will be calculated separately.
...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.options.info.Comment = ['Takes in X number of sleep spindles (each input data file = one spindle) and calculates the following parameters.<BR><BR>'...
                                     ...
                                     '4. Spindle symmetry which is the location of the spindle peak in percentage of the duration.<BR>'...
                                     ...
                                     'B) This process will generate 6 matrix files containing duration, frequency,<BR>'...
                                     'amplitude, symmetry, RMS and activity parameters from all input files.<BR>'...
                                     'C) Definition of spindle parameters from "Warby et al. Nat Methods. 2014 Apr;11(4):385-92."<BR>'...
                                     'D) Each file x-axis = spindle number but the units will = Time(s). Cannot be changed.<BR>'...
                                     'E) Follow the online tutorial which will take you to the GitHub README.md written by the<BR>'...
...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    for iNepochs = 1 : Nepochs
        ...
        for i = 1 : length(eegIxs)
            ...
            CountP(i,iNepochs) = length(PosP); % Count the number of peaks
        end
