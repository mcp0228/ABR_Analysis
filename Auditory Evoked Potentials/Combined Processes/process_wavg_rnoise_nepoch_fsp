...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    % current form.
        ...
        for j = 1 : Nepochs
            ...
            SpWEpoch (:,j) = WEpochFsp (:,231,j); % [Nchannels X Nepochs]
            SpEpoch  (:,j) = AllMatFsp (:,231,j); % [Nchannels X Nepochs]
        end
    ...
    AvgWEpoch = AvgWEpoch(:,[150 162 173 185 196 208 219 231 242 253 265 276 288 299 311]); % Selects 15 time points (degrees of freedom). [Nchannels X Ntime points]
    AvgEpoch  = mean(AllMatFsp,3); % Unweighted epoch averaging over epochs.
    AvgEpoch  = AvgEpoch(:,[150 162 173 185 196 208 219 231 242 253 265 276 288 299 311]); % Selects 15 time points (degrees of freedom). [Nchannels X Ntime points]
    ...
    % Calculation of Weighted L and R Channel Merging based on Weighted Residual Noise
    WRN_end   = WRN(:,end); % Selects the WRN of the last epoch
    ...
    WRN_L     = 1/(WRN_end(4,:)); % Selects the left channel and changes the RN into weighting factor
    WRN_R     = 1/(WRN_end(5,:)); % Selects the right channel and changes the RN into weighting factor
    ...
    % ===== SAVE THE RESULTS =====
    % Get the output study (Weighted Merging_RN)
    iStudy = sInputs(1).iStudy;
    % Create a new data file structure
    DataMat             = db_template('datamat');
    DataMat.F           = WMerg_RN;
